...
export default function GameStatsUpdateForm(props) {
  const {
    gameID: gameIDProp,
    gameStats: gameStatsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = gameStatsRecord
      ? { ...initialValues, ...gameStatsRecord }
      : initialValues;
    setGameID(cleanValues.gameID);
    setUserEmail(cleanValues.userEmail);
    setGameName(cleanValues.gameName);
    setGameStates(cleanValues.gameStates);
    setCreatedAt(cleanValues.createdAt);
    setErrors({});
  };
  const [gameStatsRecord, setGameStatsRecord] =
    React.useState(gameStatsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = gameIDProp
        ? await DataStore.query(GameStats, gameIDProp)
        : gameStatsModelProp;
      setGameStatsRecord(record);
    };
    queryData();
  }, [gameIDProp, gameStatsModelProp]);
  React.useEffect(resetStateValues, [gameStatsRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            GameStats.copyOf(gameStatsRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "GameStatsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Game id"
      ...
      >
        <Button
          ...
          isDisabled={!(gameIDProp || gameStatsModelProp)}
